...
class Feed(models.Model):
    ...
    low_stock_threshold = models.DecimalField(max_digits=10, decimal_places=2, default=50)
    image = models.ImageField(upload_to='feeds/', blank=True, null=True)
    is_active = models.BooleanField(default=True)
    date_added = models.DateTimeField(auto_now_add=True)
    
    def __str__(self):
        return f"{self.name} (KES {self.price} per {self.get_unit_display()})"
...
class CartItem(models.Model):
    ...
    def __str__(self):
        return f"{self.quantity} {self.feed.unit} of {self.feed.name}"
